...
public class PhotoFrameView extends View {
	...
	private float getScreenSize() {
		...
		screenSize = (float) Math.sqrt((getWidth() * getWidth()) + (getHeight() * getHeight()));
		return screenSize;
	}

	private float getMinDiff() {
		if (dx < 0) {
			dx = (float) (((int) (getScreenSize() - 10f)) / CameraController.getController().getMaxZoom());
			dx /= 3;
		
			if(dx<10f){				
				CameraController.getController().setZoomModifier((int)(10/dx));
				dx = 10f;
			}
		}
		return dx;
...
//		this.rect = CameraController.getController().getFrameCoordinates();
		...
		canvas.drawRect(rect.left, rect.bottom, canvas.getWidth(), canvas.getHeight(), p);
		...
		canvas.drawRect(rect.left, rect.top + 2, rect.left + 2, rect.bottom - 1, p);
		canvas.drawRect(rect.right - 1, rect.top, rect.right + 1, rect.bottom - 1, p);
		canvas.drawRect(rect.left, rect.bottom - 1, rect.right + 1, rect.bottom + 1, p);
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			mode = NONE; // clear action after release
			break;
